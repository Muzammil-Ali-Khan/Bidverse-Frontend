...
class UserBiddedProductsScreen extends StatefulWidget {
  const UserBiddedProductsScreen({Key? key}) : super(key: key);

  @override
  State<UserBiddedProductsScreen> createState() => _UserBiddedProductsScreenState();
}

class _UserBiddedProductsScreenState extends State<UserBiddedProductsScreen> {
  List<ProductModel> userBiddedProducts = [];
  UserProvider userProvider = UserProvider();

  Future<bool> _fetchProducts() async {
    var response = await HttpService.get("${URLS.getUserBiddedProducts}${userProvider.user!.id}", withAuth: true);

    if (response.success) {
      setState(() {
        userBiddedProducts = (response.data!['products'] as List).map((prod) => ProductModel.fromJson(prod)).toList();
      });
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            CustomAppBar(
              title: "Your Bidded Products",
              backArrow: true,
            ),
            userBiddedProducts.isEmpty
                ? SizedBox(
