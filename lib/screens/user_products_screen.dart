...
class _UserProductsScreenState extends State<UserProductsScreen> {
  ...
  Future<bool> _fetchProducts() async {
    var response = await HttpService.get("${URLS.getUserProducts}${userProvider.user!.id}", withAuth: true);

    if (response.success) {
      setState(() {
        userProducts = (response.data!['products'] as List).map((prod) => ProductModel.fromJson(prod)).toList();
      });
